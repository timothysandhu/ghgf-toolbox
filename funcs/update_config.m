function out_config = update_config(config,param_name,mu_or_sa,new_value)
...
target_field = strcat(param_fields{found_p},mu_or_sa);
...
function [sentence_array] = make_free_param_text(param_inds,names,nums,trans)
...
for i=1:length(param_inds)
    ...
    i_nums = nums(p_ind);
    i_trans = trans(p_ind);
    sentence_array(i) = strcat(i_name," at level ",num2str(i_nums)," is free.");
    if i_trans ~= ""
        sentence_array(i) = strcat(sentence_array(i)," Note this parameter is estimated in ",i_trans,"space.");
    end
end
