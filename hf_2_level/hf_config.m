...
% is centered at 0.
c.mu_0mu = [NaN, 0, 0];   % mu_3(0) fixed at 0 (level 3 frozen)
c.mu_0sa = [NaN, 1, 0];   % mu_3(0) not estimated

c.logsa_0mu = [NaN, log(0.1), log(0.001)]; % sa_3(0) fixed at ~0 (level 3 frozen)
c.logsa_0sa = [NaN, 1, 0];                 % sa_3(0) not estimated 
...
% Format: row vector of length n_levels.
% Undefined (therefore NaN) at the first level.
c.ommu = [NaN,  -5, -20]; % omega_3 fixed at -20: exp(-20)~=0, no random walk at level 3
c.omsa = [NaN, 2^2,   0]; % omega_3 not estimated
